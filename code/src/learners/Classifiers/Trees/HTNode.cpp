...
//	for (int i = 0; i < attrObsSize; i++, iter++) {
    ...
    /*AttributeClassObserver* observers_array [attributeObservers->size()];
    for (int i = 0; i < attributeObservers->size(); i++, iter++) {
        observers_array[i] = (*iter);
    }

    int n = inst->getNumberInputAttributes();

    #pragma acc enter data create(inst, ht, observers_array[0:])

    #pragma acc parallel loop
    for (int i = 0; i < n; i++) {
        #ifdef USE_ATOMIC
		int instAttIndex = i;
		AttributeClassObserver* obs = observers_array[i];
		if (obs == nullptr) {
			obs = inst->getInputAttribute(instAttIndex)->isNominal() ?
					ht->newNominalClassObserver() :
					ht->newNumericClassObserver();
			//observers_array[i] = obs;
		}
		obs->observeAttributeClass(inst->getInputAttributeValue(instAttIndex),
				(int) inst->getLabel(), inst->getWeight());
        observers_array[i] = obs;
    ...
    iter = (*attributeObservers).begin();
    for (int i = 0; i < attributeObservers->size(); i++, iter++) {
        (*iter) = observers_array[i];
    }*/
}
...
list<AttributeSplitSuggestion*>* ActiveLearningNode::getBestSplitSuggestions(
    ...
    int n = attributeObservers->size();

    #pragma acc enter data create(observers_array, ht, criterion, preSplitDist, bestSuggestions)
...
vector<double>* LearningNodeNB::doNaiveBayesPrediction(const Instance* inst,
    ...
    int n = observedClassDistribution.size();

    #pragma acc enter data create(inst, attributeObservers)

    #pragma acc parallel loop
	for (unsigned int classIndex = 0;
			classIndex < n; classIndex++) {
        //#ifdef USE_ATOMIC
